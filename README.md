...
- 완전 무료, 별도 가입 불필요
- Melon / Genie / Spotify 플레이리스트 URL 입력만으로 자동 변환 (멜론 단축 URL 포함)
- 곡 수 제한 없음 (멜론 50곡 이상 페이지네이션 자동 처리)
...
3. 음악 플랫폼 선택 후 플레이리스트 URL 입력
   ...
   **Genie**
   - `https://www.genie.co.kr/myMusic/myfolder?mxnm=...`
   - Genie에 로그인된 탭을 미리 열어두어야 합니다

   **Spotify**
   - `https://open.spotify.com/playlist/...`
   - 변환 중 Spotify 탭이 자동으로 열리고, 전체 곡 목록을 가져오기 위해 스크롤이 자동으로 움직입니다 (곡 수에 따라 수 초~2분 소요). 완료되면 원래 보던 탭으로 자동 복귀합니다
...
- Genie는 로그인된 탭이 열려 있어야 합니다 (비공개 보관함도 변환 가능)
- Spotify 플레이리스트는 공개 상태여야 합니다. 변환 중 탭이 자동으로 열리고 스크롤되니 다른 작업에 사용 중인 탭이 아닌지 확인하세요
...
외부 서버와 통신하지 않습니다.
멜론, Genie, Spotify, 유튜브 외의 서비스로 어떠한 데이터도 전송되지 않으며,
